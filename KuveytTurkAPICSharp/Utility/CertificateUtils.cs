...
namespace KuveytTurkAPICSharp.Utility
{
    public class CertificateUtils
    {
        ...
        /// <returns></returns>
        public static string CreateSignature(String pemstr, string accessToken, string content)
        {
            var rsaProvider = GetRSAProviderFromPemFile(pemstr);
            byte[] plainTextBytes = Encoding.UTF8.GetBytes(accessToken + content);
            byte[] hash = rsaProvider.SignData(plainTextBytes, HashAlgorithm.Create("SHA256"));
            var data = Convert.ToBase64String(hash);
            return data;

        }
        /// <summary>
        /// Get RSAProvider From Pem File
        /// </summary>
        /// <param name="pemstr"></param>
        /// <returns></returns>
        static RSACryptoServiceProvider GetRSAProviderFromPemFile(String pemstr)
        {
